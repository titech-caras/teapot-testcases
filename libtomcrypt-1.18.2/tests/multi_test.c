...
len2 = sizeof(buf[0]);
omac_memory_multi(find_cipher("aes"), key, 16, buf[1], &len2, global_buf, size, NULL);

len2 = sizeof(buf[0]);

if(size>10){
   char s1[5];
   strncpy(s1, global_buf, 5);
   omac_memory_multi(find_cipher("aes"), key, 16, buf[1], &len2, s1, 5, global_buf, size, NULL);
}
